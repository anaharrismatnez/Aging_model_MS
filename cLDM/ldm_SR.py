...
def main(args,device):
    ...
    train_data = train_dataset(data,'SR',transform=rest_SR)
    ...
    for ep in range(start_epoch,args.e):
        ...
        for i,data in tqdm(enumerate(train_loader),total=len(train_loader)):
            ...
            timesteps = torch.randint(0, config['ldm']['scheduler']['num_train_timesteps'], (HR.shape[0],), device=device).long()
            LR_timesteps = torch.randint(0, max_noise_level, (HR.shape[0],), device=device).long()

            optimizer.zero_grad(set_to_none=True)
            with autocast(enabled=True):
                with torch.no_grad():
                    e = autoencoder.encode_stage_2_inputs(HR) * scale_factor
                    e_c = autoencoder.encode_stage_2_inputs(LR) * scale_factor
            
                noise = torch.randn_like(e).to(device)
                LR_noise = torch.randn_like(e_c).to(device)
                noisy_e =  scheduler.add_noise(original_samples=e,noise=noise,timesteps=timesteps)
                noisy_e_c =  scheduler.add_noise(original_samples=e_c,noise=LR_noise,timesteps=timesteps)

                combined = torch.cat((noisy_e_c,noisy_e), dim=1)
                
                noise_pred = diffusion(x=combined,timesteps=timesteps,class_labels=LR_timesteps)
        ...
        if args.w:
            ...
            #if (ep + 1) % 10 == 0:
            fig = get_figure_ldm_SR(e.shape,e_c,autoencoder,diffusion,scheduler,device,scale_factor)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-d',default='/home/extop/Ana/Datasets/Niftis_sans_Workspaces_biogen/numpys_SR_basal.pkl', type=str,help='Data path')
